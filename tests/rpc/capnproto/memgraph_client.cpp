...
int main(int argc, const char *argv[]) {
  ...
  Storage::Client storage = client.getMain<Storage>();
  ...
  {
    auto req = storage.getPropertyStreamRequest();
    req.initReq().setName("world ");
    req.getReq().setCount(12);
    auto result = req.send().wait(waitScope);
    for (const auto &resp : result.getProperties()) {
      std::cout << resp.getStringV().cStr() << '\n';
    }
  }
