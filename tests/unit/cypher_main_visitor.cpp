...
public:
  ParserTables(const std::string &query)
      : query_(query), visitor(), ds_([this]() -> const DataStructures & {
          visitor.visit(parser_.tree());
          return visitor.ds();
        }()) {}
  std::string query_;
  frontend::opencypher::Parser parser_{query_};
...
//  ASSERT_THAT(
//      node_property_keys,
//      UnorderedElementsAreArray(property_keys.begin(),
//      property_keys.end()));
//}
//
...
TEST(CompilerStructuresTest, NodePatternEmpty) {
  ...
  ASSERT_EQ(parser.ds_.patterns().size(), 1U);
...
// TEST(CompilerStructuresTest,
// MultipleRelationshipsWithVariableAndProperties)
// {
...
//  ASSERT_THROW(ParserTables parser("CREATE (a)-[a]-()"), SemanticException);
//  ASSERT_THROW(ParserTables parser("CREATE a=()-[a]-()"),
//  SemanticException);
//}
}
